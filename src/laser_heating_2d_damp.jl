...
@views function laserheating_2D_damp(; do_visu=true, save_fig=false)
    # Physics
    ...
    while t<ttot
        ...
        while err>tol && iter<itMax
            ...
            ResU   .= .-(inn(U) .- inn(Uold))/dt .+
                (.-diff(qUx, dims=1)/dx .-diff(qUy, dims=2)/dy .+ S(t)[2:end-1,2:end-1])  # rate of change
            dUdtau .= ResU .+ damp*dUdtau
            dtau   .= (1.0./(min(dx,dy)^2 ./kx ./4.1) .+ 1.0/dt).^-1  # time step (obeys ~CFL condition)
            U[2:end-1,2:end-1] .= inn(U) .+ dtau.*dUdtau      # update rule, sets the BC as U[1]=U[end]=0
            iter += 1; err = norm(ResU)/length(ResU)
        end
...
laserheating_2D_damp(; do_visu=do_visu);
