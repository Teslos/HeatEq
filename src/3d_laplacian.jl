using DiffEqOperators, DifferentialEquations,LinearAlgebra, SparseArrays, Random
...
A = Dxx+Dyy+Dzz
Q = DiffEqOperators.compose(Dirichlet0BC(Float64, length.(s))...)
...
using Plots
anim = @animate for ti in t #4th order time stepper
        global u,uold,uolder
